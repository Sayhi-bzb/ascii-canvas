...
function SidebarStandard({
  ...
  return (
    <Sidebar
      collapsible="icon"
      className={cn("z-40 transition-none", className)}
      {...props}
    >
      <SidebarHeader
        className={cn(
      ...
      <SidebarContent className={cn("gap-2", !isCollapsed && "px-2 py-2")}>
        {children}
      </SidebarContent>

      {footer && <SidebarFooter className="p-2">{footer}</SidebarFooter>}
    </Sidebar>
  );
}
